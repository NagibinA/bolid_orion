...
from .const import DOMAIN, DPLS_DEVICE_TYPES, STATUS_CODES
...
class BolidOrionCoordinator(DataUpdateCoordinator):
    ...
    async def _async_setup(self):
        """Однократная инициализация при запуске."""
        _LOGGER.info("Запуск начального сканирования устройств")
        await self._scan_orion_devices()
    ...
    async def _scan_orion_devices(self):
        ...
        # После сканирования Orion, сканируем DPLS для найденных КДЛ
    ...
    async def _process_orion_response(self, response: str, expected_addr: int):
        ...
        try:
            ...
            if rsp_type != 0:  # Только ответы на команду 13
                return
            ...
            from .const import ORION_DEVICE_TYPES
    ...
    async def _scan_dpls_line(self, kdl_address: int):
        """Синхронное сканирование DPLS линии."""
    ...
    async def _process_dpls_response(self, response: str, kdl_addr: int, requested_addr: int):
        ...
        try:
            rsp_type = int(parts[2])
            if rsp_type != 58:  # Только ответы на команду 57
                return
            ...
            if device_exists != 0 and dpls_type != 0:
                ...
                _LOGGER.info(f"Найден DPLS: КДЛ {kdl_addr}, DPLS адрес {requested_addr}, тип {device_name}")
    ...
    async def _process_status_response(self, response: str, device_key: str):
        """Обработка ответа на команду 25."""
        parts = response.strip().split()
        if len(parts) < 5:
            return
